  ...
  <div class="page-container">
    <div class="table-list-search">
      <el-form ref="listQuery" :model="listQuery" inline size="mini">
        ...
        <!-- <el-form-item class="table-search-button">
          <el-button size="small" type="primary" class="el-icon-download"> 模板下载</el-button>
        </el-form-item> -->
        <el-form-item>
    ...
    <div class="table-list">
      ...
      >
        ...
        <el-table-column prop="designMan" label="汇总人" width="80" align="center" />
        <el-table-column prop="approvalTime" label="汇总时间" width="140" align="center" />
        <el-table-column prop="status" label="汇总状态" width="80" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.status === 1">未汇总</span>
            <span v-if="scope.row.status === 2">已汇总</span>
          </template>
        </el-table-column>
      ...
      <div class="dialog-body">
        <el-form :model="addForm" size="small" label-width="140px">
          <el-collapse v-model="checkCut" accordion>
            ...
            <el-collapse-item title="初步问题汇总" name="2">
              <el-row>
                ...
                <table class="detail-table">
                  <tr>
                    <td rowspan="2">序号</td>
                    <td colspan="2">存在的问题</td>
                    <td rowspan="2">原因分析</td>
                    <td rowspan="2">有关建议</td>
                    <td rowspan="2">操作</td>
                  </tr>
                  <tr>
                    <td>问题类别</td>
                  ...
                  <tr v-for="(item, index) in addForm.workTaskArr" :key="item.id">
                    <td>{{ index + 1 }}</td>
                    <td>
                      <el-input v-model="item.questionType" placeholder="点击选择指定" @focus="chooseClerk" />
                    </td>
                    <td>
                      <template v-for="(param, indexCode) in item.question">
                        <el-input :key="indexCode" v-model="param.questionNum" style="margin-bottom:5px;" />
                      </template>
                      <el-button size="mini" icon="el-icon-circle-plus-outline" circle @click="questionAdd(index+1)" />
                    </td>
                    <td>
                      <el-input />
                    </td>
    ...
    >
      <div class="dialog-body">
        <div class="table-list-search">
          <el-form ref="listQuery" :model="listQuery" inline size="mini">
            <el-form-item label="问题" prop="companyName">
              <el-input v-model="listQuery.companyName" />
            </el-form-item>
            <el-form-item label="建议" prop="companyName">
              <el-input v-model="listQuery.companyName" />
            </el-form-item>
            <el-form-item>
              <el-button size="mini" type="primary" icon="el-icon-search">查 询</el-button>
        ...
        <div class="table-list">
          ...
          >
            ...
            <el-table-column prop="serverBranch" label="问题" align="center" />
            <el-table-column prop="department" label="建议" align="center" />
...
export default {
  ...
  methods: {
    ...
    questionAdd(index) {
      ...
      this.newArr = this.addForm.workTaskArr.filter(item => {
        return item.id === index
      })
      this.reverseArr = this.addForm.workTaskArr.filter(item => {
        return item.id !== index
      })
      this.newArr[0].question.push(timepart)
      this.addForm.workTaskArr = this.newArr.concat(this.reverseArr)
    },
    // 添加
    taskpartAdd() {
      const timepart = {
